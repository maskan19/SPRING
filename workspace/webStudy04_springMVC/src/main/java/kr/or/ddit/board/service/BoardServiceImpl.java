...
//		logger.info("ex{}, ", Arrays.toString(example));
	}
	...
	private int deleteFileProcesses(BoardVO board) {
		...
		if (delAttNos != null && delAttNos.length > 0) {
			List<String> saveNames = attatchDAO.selectSaveNamesForDelete(board);
			// 첨부파일의 메타 데이터 삭제
		...
		if (savedPass.equals(inputPass)) {
			...
			if (attatchList.size() > 0) {
				...
				for (int i = 0; i < delAttNos.length; i++) {
					delAttNos[i] = attatchList.get(i).getAtt_no();
				}
