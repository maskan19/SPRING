...
public class AuthorizationFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		Map<String, String[]> securedResources = (Map) application.getAttribute(AuthenticationFilter.RESOURCEMAPNAME);
		...
		if(secured) {
			...
			if(Arrays.binarySearch(resRoles, userRole)<0) {//유저의 권한이 없다면
				pass=false;
			}
...
//			resp.sendError(HttpServletResponse.SC_FORBIDDEN);//403 접근 불가
			resp.sendError(statusCode,
					String.format("%s 자원에 대한 권한이 없음", uri));//401 권한 없음
		}
